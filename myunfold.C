...
void myunfold()
{
    ...
    TFile *f = new TFile("myunfold.root","recreate");
    ...
    for(int ir=0;ir<100000000;ir++){
        ...
        double qp=(q1+q4)/2;
        double qm=(q1-q4)/2;
        int v_index=P->FindBin(v1)*21+T->FindBin(0.0);
        int q_index=P->FindBin(qp)*21+T->FindBin(the+0.0);
    ...
    for(int ir=0;ir<10000000;ir++){
        ...
        double qm=(q1-q4)/2;
        int v_index=P->FindBin(v1)*21+T->FindBin(0.0);
        int q_index=P->FindBin(qp)*21+T->FindBin(the);
    ...
    response.Write();
    RooUnfoldBayes unfold (&response, Uq, NITER);
    
    TH1D* hReco;
    hReco = (TH1D*) unfold.Hreco();
    hReco->Write();
    TH1D*h=(TH1D*)hReco->Clone("test");
    h->SetTitle("test");
    h->Write();
    histM->Write();
